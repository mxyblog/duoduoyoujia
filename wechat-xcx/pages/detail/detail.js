...
Page({
  ...
  getGoodsSign:function()
  {
    wx.cloud.callFunction({
      name: "pquery",
      data: {
        keyword: this.data.goods_id,
      }
    })
    .then(res => {
      console.log(res)
      let goodList = res.result.data.goods_search_response.goods_list;
      console.log(goodList);
      this.data.goods_sign = goodList[0].goods_sign;
      this.reloadData()
    }).catch(err => {
      console.log(err)
    });
...
　　　imageUrl: this.data.detail.goods_thumbnail_url,
  ...
  buy() {
    ...
    wx.cloud.callFunction({
      ...
      data: {
        generate: true,
        goods_sign: that.data.goods_sign,
        search_id: that.data.search_id,
      }
    }).then(res => {
      console.log(res);
      wx.hideLoading();
